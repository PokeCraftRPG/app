...
internal static class MessageExtensions
{
  ...
  public static SentMessage ToSentMessage(this SentMessages sentMessages, ContactModel contact)
  {
    string confirmationNumber = sentMessages.GenerateConfirmationNumber();

    if (contact is EmailModel email)
    {
      return new SentMessage(ContactType.Email, email.Address, confirmationNumber);
    }
    else if (contact is PhoneModel phone)
    {
